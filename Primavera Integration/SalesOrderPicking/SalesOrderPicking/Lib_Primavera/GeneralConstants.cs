...
namespace SalesOrderPicking.Lib_Primavera {
    ...
    public static class GeneralConstants {
        ...
        /*
         * Condições:
         *      - Não ter picking pendente
         *      - Não estar satisfeita na totalidade (quantidade pedida > quantidade satisfeita)
         *      - Todas as linhas da encomenda têm de ter como armazém e localização o de expedição
         *      - 
         */
        public const string QUERY_ENCOMENDAS_PARA_PICKING = "SELECT CabecDoc.* FROM PRIDEMOSINF.dbo.CabecDoc INNER JOIN (" +
                                                ...
                                                ") AND NumDoc NOT IN (SELECT DISTINCT NumDoc FROM PRIDEMOSINF.dbo.CabecDoc INNER JOIN PRIDEMOSINF.dbo.LinhasDoc ON (CabecDoc.id = LinhasDoc.IdCabecDoc) WHERE (LinhasDoc.Armazem != '" + ARMAZEM_EXPEDICAO + "' OR LinhasDoc.Localizacao != '" + ARMAZEM_EXPEDICAO + "') AND CabecDoc.TipoDoc = 'ECL' AND CabecDoc.Filial = @0@ AND CabecDoc.Serie = @1@)) AS DocumentosValidos ON (CabecDoc.NumDoc = DocumentosValidos.NumDoc) WHERE CabecDoc.TipoDoc = 'ECL' AND CabecDoc.Filial = @0@ AND CabecDoc.Serie = @1@";

        /*
         * Condições:
         *      - Ter sido sujeita, pelo menos, uma vez ao processo de picking.
         *      - Todas as linhas da encomenda não podem estar associadas a uma picking wave pendente
         *      - Não estar expedida na totalidade (isto é, não satisfeita na totalidade)
         */
        public const string QUERY_ENCOMENDAS_PARA_GUIA_REMESSA = "SELECT CabecDoc.* FROM PRIDEMOSINF.dbo.CabecDoc INNER JOIN (" +
        ...
        public const string QUERY_ENCOMENDAS_ORDENADAS = "SELECT CabecDoc.*, DataMinima FROM PRIDEMOSINF.dbo.CabecDoc INNER JOIN (" +
